...
const HEFENG_API_KEY: string = process.env.HEFENG_API_KEY!;
 ...
 * @returns 城市的位置
 */
async function getCityLocation(city: string): Promise<string | null> {
  // 参照第三方服务商文档
...
/**
 *
 * @param args 包含城市和日期的参数对象
 */
export async function getWeather(args: WeatherArgs): Promise<string> {
  ...
  try {
    ...
    const res = await fetch(url);
    const data: WeatherResponse = await res.json(); // 拿到的是一周的天气
